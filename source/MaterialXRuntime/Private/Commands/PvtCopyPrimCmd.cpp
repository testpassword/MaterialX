...
RtPrim PvtCopyPrimCmd::createPrimCopy(const RtPrim& prim, const RtPath& parentPath)
{
    ...
    const PvtPrim* primPtr = PvtObject::ptr<PvtPrim>(prim);

    // Copy metadata that don't exists by default.
    copyAttributes(primPtr, PvtObject::ptr(copy));
    ...
    if (ng)
    {
        ...
        for (size_t i = 0; i < primPtr->numInputs(); ++i)
        {
            const PvtInput* port = primPtr->getInput(i);
            ...
            copyAttributes(port, PvtObject::ptr(portCopy));
        }
        for (size_t i = 0; i < primPtr->numOutputs(); ++i)
        {
            const PvtOutput* port = primPtr->getOutput(i);
...
void PvtCopyPrimCmd::copyAttributes(const PvtObject* src, PvtObject* dest)
{
