...
{
    RtPrim parent = _upstream.getParent();
    if (_upstream && parent.numInputs() > 0)
    {
        // Traverse to the first upstream edge of this prim
        // and push the next input on the stack.
        RtInput input = parent.getInput(0);
        _stack.emplace_back(_upstream, InputIndex(parent,1));
    ...
    while (true)
    {
        ...
        const RtPrim& prim = parentFrame.second.first;
        size_t& index = parentFrame.second.second;
        while (index < prim.numInputs())
        {
            RtInput input = prim.getInput(index);
            ++index;
...
{
    static RtGraphIterator NULL_ITERATOR;
    return NULL_ITERATOR;
}
